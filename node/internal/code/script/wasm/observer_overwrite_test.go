...
// the silent-overwrite defect in the wasm package's global observer: a second
...
func TestSetObserverWasmNilResetAlwaysAllowed(t *testing.T) {
	defer SetObserver(nil)
	...
	SetObserver(nil)
	// Non-nil registration from noop is fine.
	SetObserver(minObserver{})
}
