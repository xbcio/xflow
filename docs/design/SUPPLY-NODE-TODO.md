...
| 2 | Task 15 `UnregisterConsumer` 不翻转 accepted | `1d7378f` 把 readiness 改成派生合取而消除 |
| 3 | Task 15 `Registry.Ready` 与 `IsReady` 分叉 | `c4861f6` 把 `Ready` 重写为基于 `IsReady` |
| 4 | Task 17 doom 的 timeout 分类无承重测试 | `ec49765` 已修 |
| 5 | Task 17 `OnPoolSwap` 注释列了不存在的 `source_error` | `ec49765` 已修 |
| 6 | Task 17 `node/wasm_observer_test.go` gofmt | `ec49765` 已修 |
