...
| `c7d6bc3` | `sdk/runner/config.go`、`main_test.go` | 让配置样例过门禁 |
...
按「不做会怎样」排序。**六条现已全部结案**——§4 的两半（过期、使用次数上限）现在都已落地，
但**两者都默认关闭**，所以默认配置下的行为与原文逐字一致；§6 结的方式是加守卫而不是消除
披露（那条披露本身是 `list_global` 这个 scope 的用途，不是缺陷）。
...
### 4. ~~注册码本身仍没有过期机制~~ 已修（两个放大器都已拆）

> **两个放大器现已都拆掉**：过期见 `feat(enroll): give registration codes a lifetime`，
> 使用次数上限见本条末尾。本条保留原文，因为它记录的是这两个放大器**互相独立**——
> 设了时限不等于限了次数，限了次数也不等于设了时限，而且哪一个都不能替代吊销。
>
> **两者都默认关闭**（`ExpiresAt` 零值＝永不过期，`MaxUses == 0`＝不限次数）。所以一枚
> 什么都没设的注册码，行为与本条原文描述的**逐字一致**。这不是遗漏：让升级追溯作废或
> 追溯限制已经发出去的码，是比「默认没上限」更坏的一种意外。
>
...
>
> **使用次数上限（第二个放大器，已修）**，落点：`RegistrationCode.MaxUses` / `UseCount` +
> `IsExhausted()`（`>=` 而不是 `==`：万一 `MaxUses` 事后被调小、`UseCount` 已经越过了
> 天花板，那一行要读作「已用尽」，而不是绕回「不限次数」）。
>
> 计数**不在 `ResolveByPlaintext` 里做**，而是新开一个 `Consume(ctx, id)`。理由有两层：
> 一是查询不该改状态（会破坏 `TestMemoryStoreReturnsDefensiveCopies`）；二是被 scope 检查
> 拒掉的 enroll **不能**花掉码的一次配额——把计数塞进查询就做不到这一点。
>
> SQL 侧是一条**条件 UPDATE + `RowsAffected`**（`WHERE id=? AND revoked=false AND
> (max_uses = 0 OR use_count < max_uses)`），照 `issuedIdentityRepo.Renew` 的既有先例：
> 读与写是同一条语句，所以两个并发 enroll 争最后一次配额不可能都赢——那正是这一列存在的
> 全部意义。把 `max_uses = 0 OR ...` 挪回 Go 里判断就会丢掉这个原子性。
>
> 一条 UPDATE 分不清 `RowsAffected == 0` 是「已吊销」「已用尽」还是「不存在」，所以这里
> **把执行与归类拆开**：拒绝是原子的，归因是随后一次不加锁的读，只用来填审计 `reason`。
> 三种原因对外统一收敛成 `ErrEnrollRejected`，所以极端并发下归错类的代价，只是一行不够
> 精确的审计记录。引入事务来消除它，复杂度远超收益（与「吊销的 TOCTOU」同一判断）。
>
> `Consume` 在 `Issue` **之前**调用，故意是 fail-closed 的：两者之间崩溃会白烧一次配额
> （在 `use_count` 里看得见、重发一枚码即可恢复），反过来则会在「天花板正是为之存在的
> 那种崩溃」上静默超发。
>
> API 侧：`max_uses` 默认 0＝不限次数，**没有部署级天花板**（与 `expires_in_seconds` 不同），
> 所以它是普通 `int` 而不是指针——「没传」与「显式传 0」是同一个请求。将来若真加了天花板，
> 这个字段必须同时改成指针。负值一律 400 而不是夹到 0：夹的话，「我客户端有 bug 传了 -1」
> 会被静默变成「这枚码完全没有上限」，正好是最宽的那个结果。视图字段 `max_uses` / `use_count`
> 刻意**不带 `omitempty`**——省掉的话，「不限次数」与「服务端老到没有这个字段」就分不出来了。
...
`sdk/runner/renew_test.go` 只驱动这个纯函数本身、断言它返回的四个值，没有驱动 `runRunner`
全程去观察续期 goroutine 是否真的按判定结果被启动或不被启动。

判定与启动之间的实际距离（`sdk/runner/run.go` 的 `runRunner` 续期分支）：
...
### ~~注册码可复用~~ 已修（改为可设上限，默认仍不限）

> **不再是一条被接受的代价**，见「待办 §4」末尾。本条保留原文，因为它是
> `max_uses` / `use_count` 这两列存在的**唯一理由**，也因为原文描述的行为在默认配置下
> **依然逐字成立**——修法给的是一把可选的闸，不是一个新的默认值。
>
> 需要补记的一点：这条代价当初被接受时，**没有人写下复用的理由**。`3a3d57c` 的提交正文
> 说明当时 OpenAPI 契约写的是 one-time-use、而实现从来不是——也就是说「可复用」不是一个
> 被权衡后选中的设计，是一处没被发现的契约与实现分叉。默认保持不限次数，理由与默认永不
> 过期相同（升级不得追溯限制已发出的码），另外「一枚码装一整支机队」是已经写进 OpenAPI
> 的公开行为，改默认值等于毁约。
>
> **刻意不做**：部署级的 `--registration-code-max-uses` 天花板（对应 `--registration-code-ttl`
> 那一半）。做了它才能约束 `registration_code.create_global` 持有者在这条轴上的自由度。
> 触发条件写在代码里：`registrationCodeCreateRequest.MaxUses` 的注释点名了「一旦加天花板，
> 这个字段必须同时改成指针」。
...
样例默认倒向前者。两条测试守卫锁住这个形状（`sdk/runner/main_test.go`）：一条断言样例能通过
