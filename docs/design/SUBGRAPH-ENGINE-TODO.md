...
修复（36e7e59）把这个窗口放大过——每次 flush 变短，两个 worker 撞上同一批条目的
...
**「列出即租约」曾是这里的方案，实测有害，已推翻（94a6db4 → 本次）。** 让读取顺手取
...
`concurrency=4` 下四个 400 批次的并行 map 就够）。已在 `36e7e59` 单独修复：
...
### `ProjectSubgraphPackage` → `ProjectGroupPackage`（2026-08-11 `a015adf`）
...
本条曾以「改名会动到公开 API，未做」挂在 P2。`a015adf` 跨 9 个文件一次改完，
...
Go 侧编译门控已在 `a617f62` 移除，`web/packages/xflow-core/src/index.ts:44` 的
...
比递归接线便宜得多」。**该建议未被采纳，实际走了递归接线那条路**（`3cfa1db`），
...
| P2-8 | `ProjectSubgraphPackage` 名字有歧义 | `a015adf`（重命名为 `ProjectGroupPackage`） |
