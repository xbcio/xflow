...
- **Local backend (`backend/providers/local`)**
...
- **Asynq backend (`backend/providers/distributed`)**
...
- **端点默认拒绝**：`debugging_url` 的 host 必须命中 runner 进程级 `endpoints`；空 allowlist 拒绝全部端点。条目可为忽略大小写的精确 host、以 `.` 开头且包含 apex 的后缀，或以 `*.` 开头且要求至少一个额外 label 的通配符；端口不参与匹配。此 allowlist 与 `HTTPHostPolicy` 是两套独立策略，不能用其中一套替代另一套。
...
默认值为：`endpoints: []`（拒绝所有 endpoint）、`max_contexts: 1`、`queue_timeout: "5s"`、`connect_timeout: "5s"`。独立 runner 也支持以下环境变量和同名 CLI 配置：

| YAML 字段 | 环境变量 | CLI flag |
|---|---|---|
| `browser_cdp.endpoints` | `XFLOW_BROWSER_CDP_ENDPOINTS`（逗号分隔 host） | `--browser-cdp-endpoints` |
...
「该不该扩展」的判据是「编译期是否投影了 body」（`graph.BodyAt`）——它是图的结构
性质，不由 payload 回答。于是无 body 的 map 会被判定为「不扩展」，描述符被当成节点
的普通输出提交，body 一次都不跑，下游读到 `items`/`batches` 而不是
`results`/`count`——静默的错答案。编译期拒绝是这个判据成立的前提，它落在两条编译
路径共用的 pass 上（含 group 包走的受信路径）。
...
- name: process_items
  ...
  parameters:
    ...
    batch_size: int          # 每批的项数，默认 1；这是耐久单元大小，不是并发度
    continue_on_error: bool  # 单项失败是否继续
    body_concurrency: int    # 单批内最多同时跑几项，默认 1（批内串行）
...
>
> **`body_concurrency`（单批内并发，默认 1 = 串行）**（已实现，见 `execution/subgraph/map_body.go` 的 `executeConcurrently`）：
>   - `batch_size` 与 `body_concurrency` 是两个正交的量：前者决定**一批多少项**（耐久与重试单元的大小），后者决定**一批之内同时跑几项**。`batch_size: 40` 从不意味着 40 项并发——批与批之间的并发来自它们是各自独立的引擎任务
>   - 因此默认串行，且必须显式开启：一批是作者刻意定的尺寸，把它整批同时跑会把该批的峰值资源占用乘以批长——body 内含 wasm guest 时，40 项并发意味着 40 份活的线性内存而非 4 份
>   - 开启后 `continue_on_error: false` 的保证会**减弱**：串行时保证「失败项之后的项一个都不跑」，并发时只能保证「观测到失败后不再**启动**新项」。已在途的项会跑完并保留副作用（无回滚），所以有副作用的 body 成员必须以 `$item` 里的业务键做幂等
>   - 适用场景是「这一批就是唯一在途的批」——例如 Kafka trigger group 每次 flush 同步跑一个批，此时「并发来自批」不成立，项是唯一剩下的并发轴
>   - 值 ≤ 1、非数字（含未求值的表达式字符串）、缺省一律按串行处理：参数在调度期读取，那时还不存在可用于求值的逐项环境；而非正数不是上限，把它当「无上限」会让一个笔误变成成千上万个并发子执行
>
...
>
> **跨域引用运行期语义（快照，非强一致）**：
> - 外层节点的输出在**进入 map 节点、批被调度时读取一次**，整个 body 的所有迭代、所有成员看到的都是同一份值。body 内不会重新读取，也不保证与外层的后续变化一致——**不需要强一致性是刻意的设计选择**，它让快照可以随批一次性下发到 runner。
> - 快照随批下发（`SubgraphLeasePayload.OuterNodes`），因此**跨域引用在分布式模式下同样可用**：runner 看不到外层 execution 的状态存储，值必须跟着批走。
> - 投影出的 body 包只携带**允许被引用的节点名**（`SubgraphPackage.VisibleOuterNodes`），不携带输出内容——上游节点的输出常常是含凭证的 HTTP 响应体，绝不能进入编译期产物（同 `VisibleSupplies` 的理由，但更强）。
> - 编译期已排除永远不会执行的引用（下游节点、无关分支）。剩下的只有「条件分支合法跳过的祖先」，此时 `$nodes['x']` 读到 `nil`，用 spec 推荐的 `?? 默认值` 兜住即可；读取失败（存储故障）则是真正的错误，会让该批失败。
> - body 成员**同时**引用兄弟成员和外层节点时两者都可见：兄弟从内层 execution 读，外层来自快照，合并进同一个 `$nodes`。
