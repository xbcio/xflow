...
| A3 跨进程错误分类与兼容语义 | 🟡 部分完成 | wire DTO + `ClassifiedError` 完成；HTTP 408/429 transient、Database 按 SQLState/number 分类、gRPC codes 表完成；**三拓扑 parity 矩阵 harness 已完成**（local embedded + server-runner，真 Redis，transient-then-success / retry-exhausted / permanent / business-error-no-retry / error-port-retry-exhausted 五 fixture 跨拓扑 attempt/status/error-code 一致，cluster-transient 因无 handler retry 为 collection-path 排除）。**code/script/function classifier 迁移完成**（config 错误 permanent、timeout transient、用户确定性错误保留显式 error port；引擎修复 error-port retry 耗尽后走 terminal failure 而非 success-on-error-port）。⚠️ 剩余：error-taxonomy spec §6 分阶段清单未全部关闭（HTTP 4xx/5xx 全表、gRPC 全 codes、DB deadlock/connection-lost/constraint 逐 fixture 矩阵、OnError fail/continue/route 策略），清单关闭前 A3 不标 G0 完成 | `1995148`、`6f16260`、`1815635`、`987a8ff` |
