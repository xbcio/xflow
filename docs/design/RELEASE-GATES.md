...
| A3 跨进程错误分类与兼容语义 | 🟡 部分完成 | wire DTO + `ClassifiedError` 完成；HTTP 408/429 transient、Database 按 SQLState/number 分类、gRPC codes 表完成；**三拓扑 parity 矩阵已大幅扩展**（local embedded + server-runner，真 Redis）：新增 HTTP 4xx/408/429/5xx/connection、gRPC no-pool/NotFound/Unavailable/connection、DB no-pool/bad-conn/deadlock/constraint、OnError stop/error_output/main_output/continue、script/function config/timeout/user-error fixture。**gRPC status codes 全表 parity fixtures 已完成**（DeadlineExceeded/InvalidArgument/PermissionDenied/Unauthenticated/AlreadyExists/Unimplemented/FailedPrecondition/OutOfRange 等 14 个 code 跨 local/server-runner 验证）。cluster-transient 因无 handler retry 为 collection-path 排除。⚠️ 剩余唯一缺口：Database server-runner 真实 MySQL 路径；清单关闭前 A3 不标 G0 完成 | `1995148`、`6f16260`、`1815635`、`987a8ff`、`31338fb`、`681ba6f`、`6bdc794`、`bdf0ad3`、`81d6b58`、`1430bda`、`8cc51d3` |
