...
var metricHelp = map[string]string{
	...
	"xflow_trigger_offset_commit_duration_seconds":   "Wall time blocked in the broker round trip that advances the committed offset, per topic and result (ok/error). This is the trigger's only WAITING metric, and it paces the pipeline: commits are synchronous (CommitInterval=0, so the offset follows the side effect) and serialized per partition, so this sits on the critical path between one batch and the next. Not to be confused with xflow_commit_outcomes_total, which is the execution layer's commit and carries no topic.",
	"xflow_trigger_offset_commit_size":               "Offsets carried by a single commit round trip, per topic. Read against xflow_trigger_offset_commit_duration_seconds: a flat duration as this rises means the commit cost is per-round-trip and a larger aggregate max_size buys throughput; a duration that rises with it means the cost scales with the offsets committed and max_size buys nothing.",
