...
var metricHelp = map[string]string{
	...
	"xflow_entry_seed_admission_total":               "Entry-seed admission attempts on the runner (POST /v1/executions, one per trigger batch), partitioned by outcome (accepted/duplicate/conflict/timeout/error). This is the only series that separates a deadline breach from the other transport failures: xflow_trigger_batch_admissions_total reports both as state=error, reason=seed_transport, while the two call for opposite responses (lower the batch or raise the deadline, versus investigate the network). A rising timeout rate means the configured batch has outgrown the admission window, and every one of them redelivers and re-executes its whole batch.",
	"xflow_entry_seed_admission_duration_seconds":    "Duration of the entry-seed admission round trip, partitioned by outcome. Read against the configured admission window (protocol.DefaultEntrySeedRequestTimeout unless the runner raised it): admissions sitting close to that deadline are one traffic spike away from becoming redelivered batches, which the rate alone does not show. Samples in the timeout outcome pile up at the deadline, which is what distinguishes a genuine breach from a slow success.",
	"xflow_group_admission_total":                    "GROUP entry-unit admission attempts through Engine.SeedExecutionFromEntry, partitioned by outcome (accepted/duplicate/conflict/error). Only entry units resolved as group units are counted; a non-group entry unit, a nil Graph, or an out-of-range EntryUnitIdx is not observed at all, so this is a lower bound whenever group membership could not be resolved.",
	...
	"xflow_group_commit_total":                       "Group unit commits reaching a terminal result, partitioned by outcome (success/failed_fatal/failed_tolerated). A commit discarded as stale/late (CommitOutcomeExecutionInactive) is not counted here — see xflow_group_exec_duration_seconds' comment for why.",
	...
	"xflow_lease_maintenance_pass_total":             "Background maintenance passes over shared lease state, partitioned by pass (repair/legacy_lease_meta/stranded_lease/dead_queued_assignment) and outcome (ran/error/skipped_not_leader/skipped_cadence/unsupported). Only outcome=ran means the pass did its work, and it is counted whether or not anything was released — so outcome=ran with a flat xflow_lease_maintenance_released_total is \"ran and released nothing\", while a pass with no ran series at all never ran. The skipped_* outcomes name the gate that stopped it: leadership (a non-leader replica is a legitimate permanent skip) or the pass's own cadence. unsupported means the backend/directory does not implement the capability, so the pass can never run on this deployment. Before this series those three states were all silence: the passes reported only through an optional logger, which a host that injects none never sees. The namespace label is the sweeper's own context, not a tenant's — these passes span every namespace.",
	"xflow_lease_maintenance_released_total":         "Records released, reaped, reclaimed or reconciled by a background maintenance pass, partitioned by the same pass values as xflow_lease_maintenance_pass_total. Incremented once per pass that ran (outcome ran/error), including by zero, and not at all for a skipped pass. Per pass: repair counts lease-index entries reconciled, legacy_lease_meta counts orphaned pre-U-7 lease-metadata entries reaped, stranded_lease counts 'leased' assignments released whose lease metadata had expired, dead_queued_assignment counts 'queued' assignments removed whose execution was gone. Read its rate as the arrival rate of the shape that pass drains; all four passes are bounded by a batch, so a rate pinned at the batch size means the backlog is not being kept up with.",
	"xflow_lease_maintenance_candidates_total":       "Candidate records a background maintenance pass inspected, partitioned by the same pass values as xflow_lease_maintenance_pass_total. Incremented once per pass that ran (outcome ran/error), including by zero, and not at all for a skipped pass. Per pass, this is what the pass discovered in the shape it drains: legacy_lease_meta counts every field its HSCAN read from the pre-U-7 lease-metadata hash, which is that shape in its entirety; stranded_lease counts the 'leased' assignments its per-runner walks found plus the handoff-ledger records still marked finalized; dead_queued_assignment counts the assignment-state entries whose recorded state is 'queued', deliberately not every entry the hash holds, because an entry in another state is not a candidate of that shape. Read it against xflow_lease_maintenance_released_total, which is the only thing that makes either number interpretable: a pass inspecting far more than it releases is releasing only a small part of its own candidate set, which is a stale index or a scope wider than the shape the pass exists to drain, and it is equally what a batch-capped pass looks like when the batch is smaller than the backlog. Most inspected entries of dead_queued_assignment are expected NOT to be released on a healthy fleet — that pass walks the head of a live queue — so read its ratio only against whether the queue is converging. pass=repair is deliberately absent: that capability lives in the backend state store and reports a reconciled count but no candidate count, so a zero there would be an invented measurement rather than an observation.",
	...
	"xflow_outbox_ready":                             "Outbox messages due for dispatch right now, from the dispatcher's last backlog scan. The due-now subset of xflow_outbox_pending: pending also counts entries a live deliverer has leased and entries waiting out a retry backoff, neither of which this tick could hand to the queue. Large pending with small ready is a backlog of future work; pending large and comparable to ready is a backlog dispatch is failing to clear.",
	...
	"xflow_runner_drain_blockers":                    "Fleet-wide drain blockers, partitioned by bounded blocker kind.",
	...
	"xflow_supply_hint_read_errors_total":            "Supply reads that failed while the control plane computed heartbeat hints, partitioned by namespace and cause. The expected not-found case is excluded. A nonzero rate means one or more runners are not being told about a supply they host a consumer for, so they keep serving last-good content; the HTTP GET for the same row answers 500.",
	...
	"xflow_trigger_messages_discarded_total":         "Trigger messages consumed but never emitted, partitioned by topic and reason (schema/schema_fail/buffer_overflow). Any nonzero rate is data loss. buffer_overflow is the permanent kind: the aggregator dropped a message it had already read and the commit frontier still advances past it, so nothing redelivers it. That is the default (on_overflow=discard); a topic that cannot afford it sets on_overflow=block (which trades this series for xflow_trigger_consumption_blocked) or on_overflow=dead_letter (which trades it for xflow_trigger_messages_dead_lettered_total, because the record is republished rather than dropped).",
	"xflow_trigger_messages_dead_lettered_total":     "Dead-letter publish attempts, partitioned by topic and result (ok/error). Counts both dead-letter axes: message_schema's on_invalid=dead_letter (records that failed validation) and the aggregator's on_overflow=dead_letter (records the buffer was at its cap for). The axis is in the published record's xflow-dlq-reason header, not in a label, so the label set stays bounded. An error rate means the record is being redelivered rather than parked and the source partition has stopped consuming on it.",
	...
	"xflow_trigger_consumption_blocked":              "Whether this partition has stopped consuming to avoid dropping records, per topic and partition. 1 means the aggregator is at its buffer cap under on_overflow=block, or is holding a record whose on_overflow=dead_letter publish has not succeeded; either way the shared reader has stopped fetching for the WHOLE assignment until this partition progresses. Neither lag gauge can report it: a partition that stopped fetching stops sampling lag, so it holds its last healthy value.",
	"xflow_trigger_offset_commit_duration_seconds":   "Wall time blocked in the call that advances the committed offset, per topic and result (ok/error). This is the trigger's only WAITING metric, and it paces the pipeline. Despite the name it is NOT broker latency: commits are synchronous (CommitInterval=0, so the offset follows the side effect) and every partition of a Reader queues onto ONE channel drained by ONE goroutine, so this is mostly time spent waiting behind the other partitions. A high value means commit concurrency, not the brokers. Not to be confused with xflow_commit_outcomes_total, which is the execution layer's commit and carries no topic.",
	"xflow_trigger_offset_commit_size":               "Offsets carried by a single commit, per topic. Read against xflow_trigger_offset_commit_duration_seconds: because the commit rate is capped by one serial goroutine and does not scale with partitions, consumed throughput is that capped rate times this value — which makes this the only term the aggregate's max_size can move. Expect it to exceed max_size, since a partition waiting in the commit queue keeps buffering and carries the backlog on its next commit.",
	...
	"xflow_workflow_registration_total":              "Workflow registration attempts on the control plane's registration path (POST/PUT /v1/workflows and the in-process Server.AddWorkflow/ReplaceWorkflow), partitioned by operation (add/replace/replace_by_id) and outcome (registered/invalid_definition/conflict/error). A registration that fails at startup leaves the host process up, its own HTTP API working, and its xflow runner surface answering 503 forever — before this series nothing in /metrics said so. Alert on the ABSENCE of outcome=\"registered\" in a process that should have registered one, or on any non-zero invalid_definition/conflict rate: those two are deterministic, so retrying them is noise and the definition or the key has to change. Only outcome=\"error\" (backend or transport, including a context deadline) is worth retrying, which is what xflow.IsRetryableRegistrationError tells a host.",
}
