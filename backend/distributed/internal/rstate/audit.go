...
func (s *Store) auditWrite(ctx context.Context, op string, fn func(context.Context) error) {
	if s.transient {
		// Transient executions are fire-and-forget: skip the MySQL audit mirror
		// unconditionally, even when a Store is configured. Per-call sites also
		// gate on !s.transient, but this central guard makes the skip invariant
		// robust to new call sites that forget the gate.
		return
	}
	if s.db == nil {
		return
